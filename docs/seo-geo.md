...
apps/web/src/
  lib/
    ...
    locale-switch.ts — resolves target path when switching locale (uses public counterpart APIs)
  components/seo/
    JsonLd.tsx       — server component that injects <script type="application/ld+json">
  components/layout/
    LocaleSwitcher.tsx — calls /api/nav/locale-target (server) for correct TR↔EN URL
...
- All product slugs for both locales (from `lib/i18n` `productSlugs`)
- Blog post URLs: still driven by `mockBlogList` so the sitemap is non-empty in all environments; replace with `GET /api/v1/public/blog` for both locales when the CMS is the single source of truth
...
Public HTML is cached by Next.js using `next: { revalidate: N }` (seconds) on `apiFetch()` in `lib/api/public-content.ts`. Representative TTLs: homepage and blog list **30 minutes**, blog detail and product pages **2 hours**, contact **24 hours** — see `docs/caching-strategy.md` for the full table and rationale.

When the API publishes or unpublishes content, `CacheService` evicts the Spring/Redis public caches and, if `REVALIDATE_SECRET` and `WEB_APP_URL` are set, calls `POST /api/revalidate` on the Next.js app so `revalidatePath()` runs without waiting for the ISR TTL.
...
| Feature | Notes |
|--------|--------|
| Blog sitemap from API | `app/sitemap.ts` still uses `mockBlogList` for blog post URLs; extend it to list published posts per locale from the API for complete coverage. |
| Structured data for datasheets | `DigitalDocument` (or similar) for downloadable PDFs — optional once stable public file URLs are guaranteed in all environments. |
| Resources detail in locale switch | `lib/locale-switch.ts` handles static routes, blog, and product detail; other deep paths fall back to the target locale home until expanded. |
